...
TEST_F(
    an_active_mccp_client,
    receives_compressed_data_after_remote_engages_compression)
{
    client_.subnegotiate({});

    auto const test_data = "test_data"_tb;
    compress_decompress(
        test_data, [&](telnetpp::bytes data) { receive_data(data); });
...
TEST_F(
    an_active_mccp_client,
    with_engaged_compression_reports_compression_end_and_no_longer_decompresses)
{
    client_.subnegotiate({});
    ...
    std::vector<telnetpp::byte> const expected_data0{
        test_data.begin(), test_data.end()};
    ...
    receive_data(test_data);
    std::vector<telnetpp::byte> const expected_data1{
        test_data.begin(), test_data.end()};
...
TEST_F(
    an_active_mccp_client,
    deactivated_locally_receives_decompressed_data_until_negotiation_is_complete)
{
    client_.subnegotiate({});
    client_.deactivate();
...
TEST_F(
    an_active_mccp_client, deactivated_remotely_stops_decompressing_immediately)
{
    client_.subnegotiate({});
    client_.negotiate(telnetpp::wont);
