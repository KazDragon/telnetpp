...
protected:
    void async_read()
    {
        session_.async_read([this](telnetpp::bytes content) {
...
{
    session_.install(telnetpp::ayt, [this](telnetpp::command const &cmd) {
    ...
    session_.install(telnetpp::dm, [this](telnetpp::command const &cmd) {
    ...
    static telnetpp::byte_storage const expected_result = {'Y', 'e', 's'};
...
TEST_F(a_session, routes_subnegotiations_to_installed_client_option)
{
    ...
    client.on_subnegotiation.connect([&](telnetpp::bytes data) {
        received_subnegotiation.append(data.begin(), data.end());
    });
    ...
    telnetpp::byte_storage const expected_subnegotiation = {'T', 'E', 'S', 'T'};
...
TEST_F(a_session, routes_fragmented_subnegotiations_to_installed_client_option)
{
    ...
    client.on_subnegotiation.connect([&](telnetpp::bytes data) {
        received_subnegotiation.insert(
            received_subnegotiation.end(), data.begin(), data.end());
    });

    telnetpp::byte const content0[] = {
        telnetpp::iac, telnetpp::sb, client.option_code(), 'T', 'E'};

    telnetpp::byte const content1[] = {'S', 'T', telnetpp::iac, telnetpp::se};
...
{
    static telnetpp::byte_storage const content = {'T', 'E', 'S', 'T', 0xFF};
...
TEST_F(a_session, can_receive_data_piecemeal)
{
    ...
    client.on_subnegotiation.connect([&](telnetpp::bytes content) {
        client_subnegotiation_content.append(content.begin(), content.end());
    });

    telnetpp::byte_storage server_subnegotiation_content;
    server.on_subnegotiation.connect([&](telnetpp::bytes content) {
        server_subnegotiation_content.append(content.begin(), content.end());
    });
    ...
    ASSERT_EQ(expected_result, channel_.written_);
    ASSERT_EQ(
        expected_client_subnegotiation_content, client_subnegotiation_content);
    ASSERT_EQ(
        expected_server_subnegotiation_content, server_subnegotiation_content);
