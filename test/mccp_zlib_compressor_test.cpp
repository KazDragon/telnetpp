...
protected:
    ...
    {
        zlib_compressor_(
            data, [&](telnetpp::bytes data, bool compression_ended) {
...
TEST_F(a_started_zlib_compressor, compresses_received_data)
{
    ...
    auto const output_data = telnetpp::bytes{output_buffer, stream.next_out};
...
TEST_F(a_started_zlib_compressor, can_compress_large_amounts_of_data)
{
    ...
    while (stream.avail_in != 0)
    {
        ...
        decompressed_data.insert(
            decompressed_data.end(), output_buffer, stream.next_out);
    }
