...
protected:
    a_server_option()
    {
        server_.on_state_changed.connect([this]() { state_changed_ = true; });

        server_.on_subnegotiation.connect([this](telnetpp::bytes data) {
    ...
    fake_server_option server_{session_, option};
...
TEST_F(a_deactivated_server_option, on_do_responds_with_will_and_is_active)
{
    ...
    telnetpp::byte_storage const expected_written = {
        telnetpp::iac, telnetpp::will, option};
...
TEST_F(
    a_deactivated_server_option, on_dont_responds_with_wont_and_is_not_active)
{
    server_.negotiate(telnetpp::dont);

    telnetpp::byte_storage const expected_written = {
        telnetpp::iac, telnetpp::wont, option};
